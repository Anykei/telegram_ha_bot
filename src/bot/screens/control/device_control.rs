...
pub async fn render(
    ...
    let dev_db = crate::db::devices::get_device_by_id(device_id, db)
        .await?
    ...
    match smart_obj {
        ...
        SmartDevice::Climate(e) => super::climate_view::render(ctx, room_id, dev_db, e).await,
        // SmartDevice::Light(e) => {
        //     super::device_screens::light::render(ctx, room_id, dev_db, e).await
        // }
        // ... другие типы
        _ => {
            crate::bot::screens::common::in_dev_menu(
                ctx,
                Payload::Control(crate::bot::router::ControlPayload::RoomDetail { room: room_id }),
            )
            .await
        }
    }
