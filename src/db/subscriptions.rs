...
pub async fn is_hidden(entity_id: &str, pool: &SqlitePool) -> anyhow::Result<bool> {
    ...
    let hide_value: Option<i64> =
        sqlx::query_scalar("SELECT hide FROM hidden_entities WHERE entity_id = ?")
            .bind(entity_id)
            .fetch_optional(pool)
            .await?;
...
pub async fn toggle_hidden(entity_id: &str, pool: &SqlitePool) -> anyhow::Result<bool> {
    // Step 1: Check current state
    let current_hide: Option<i64> =
        sqlx::query_scalar("SELECT hide FROM hidden_entities WHERE entity_id = ?")
            .bind(entity_id)
            .fetch_optional(pool)
            .await?;
    ...
    match current_hide {
        Some(hide_val) => {
            ...
            sqlx::query("UPDATE hidden_entities SET hide = ? WHERE entity_id = ?")
                .bind(new_hide)
                .bind(entity_id)
                .execute(pool)
                .await?;
        ...
        None => {
            ...
            sqlx::query("INSERT INTO hidden_entities (entity_id, hide) VALUES (?, 0)")
                .bind(entity_id)
                .execute(pool)
                .await?;
